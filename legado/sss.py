...
# Game loop
while True:
    lista_pings = [jsf_pings[k]]
    k += 1  # Incremento o ping
    current_img = jsf_data.make_waterfall_image(lista_pings)
    current_img = np.array(current_img)  # Pedaço de imagem vinda do ping

    mascara = np.zeros_like(current_img)
    cor_laranja = np.array([255, 165, 0], dtype=np.uint8)
    indices_brancos = np.where((current_img == [255, 255, 255]).all(axis=2))
    mascara[indices_brancos] = cor_laranja
    intensidade = 0.4 #intensidade da cor de overlay
    current_img = (1 - intensidade) * current_img + intensidade * mascara
