...
class App(customtkinter.CTk):
    ...
    def activate_control(self): #Cria o frame com o controle
        ...
        self.control = customtkinter.CTkLabel(master=self.slider_progressbar_frame, text="Controle Remoto")
        self.control.configure(font=("Segoe UI", 30))
        self.control.grid(row=0, column=0, columnspan=1, padx=(10,0), pady=(10,30), sticky="")
        ...
        # Barra de Progresso do Leme
        self.rudder_progressbar = customtkinter.CTkProgressBar(self.slider_progressbar_frame,width=300)
        self.rudder_progressbar.grid(row=5, column=0, padx=(20, 10), pady=(0, 0),sticky="nsew")
        
        self.rudder_slider = customtkinter.CTkSlider(self.slider_progressbar_frame, from_=-40, to=40, number_of_steps=80)
        self.rudder_slider.grid(row=4, column=0, padx=(20, 10), pady=(15, 15), sticky="ew")
        ...
        #self.label_thrust.grid(row=1, column=1, columnspan=1, padx=(190,0), pady=(20,20), sticky="")

        # Barra de Progresso de Máquina
        ...
        #self.label_gear.grid(row=1, column=4, columnspan=1, padx=(10,0), pady=(10,20), sticky="")

        # Barra da Marcha
        #self.gear_slider = customtkinter.CTkSlider(self.slider_progressbar_frame, from_=-1, to=1, number_of_steps=2, orientation="vertical",height=200)
        #self.gear_slider.grid(row=0, column=4, padx=(20, 10), pady=(15, 15), sticky="ew")
        #self.bind("<a>", self.change_gear('Ré')) # Configuração para teclas de "a,s,d"
        #self.bind("<s>", self.change_gear('Neutro')) # Configuração para teclas de "a,s,d"
        #self.bind("<d>", self.change_gear('Avante')) # Configuração para teclas de "a,s,d"

        # Configurando a barra de progresso
        self.rudder_slider.configure(command=self.update_value_rudder()) #Configurei o slider para setar a barra de progresso
        ...
        #self.label_thrust_value = customtkinter.CTkLabel(master=self.slider_progressbar_frame, text=str(int(self.thrust_slider.get()*100))+"%")
        #self.label_thrust_value.configure(font=("Segoe UI", 15))
        #self.label_thrust_value.grid(row=0, column=2, columnspan=1, padx=(5,15), pady=(5,5), sticky="n")

        # Angulo do Leme
        self.label_rudder_value = customtkinter.CTkLabel(master=self.slider_progressbar_frame, text=str(int(self.rudder_slider.get()))+"°")
    ...
    def decrement_rudder_slider(self, event):
        current_value = self.rudder_slider.get()
        new_value = current_value - 1
        self.rudder_slider.set(new_value)
        self.update_value_rudder()
    ...
    def change_gear(self,command):
        dict_gear = {"Neutro":0,"Avante":1,"Ré":-1}
        new_value = dict_gear[command]
        self.gear_slider.set(new_value)
        self.update_value_gear(command)
        
    def optionmenu_callback(self,choice):
        print("optionmenu dropdown clicked:", choice)

    def update_value_thrust(self):
        self.thrust_progressbar.set(self.thrust_slider.get())
        self.label_thrust.configure(text=str(int(self.thrust_slider.get()*100))+"%")

        if self.controle_manual is True:
            self.comms.notify('DESIRED_THRUST', int(self.thrust_slider.get()*100),pymoos.time())
            print("DESIRED_THRUST: ", int(self.thrust_slider.get()))

    def update_value_rudder(self):
        rudder_value = self.rudder_slider.get()
        print(rudder_value)

        self.rudder_progressbar.set(rudder_value)
        self.label_rudder.configure(text=str(int(rudder_value))+"°")

        if self.controle_manual is True:
            self.comms.notify('DESIRED_RUDDER', int(rudder_value),pymoos.time())
            print("DESIRED_RUDDER: ", int(rudder_value))

    def update_value_gear(self,command):
        #self.gear_progressbar.set(self.gear_slider.get())
        self.label_gear.configure(text=str(command))
        gear_moos = {"Neutro":0,"Avante":1,"Ré":2}

        if self.controle_manual is True:
            self.comms.notify('DESIRED_GEAR', gear_moos[command],pymoos.time())
            print("DESIRED_GEAR: ", gear_moos[command])
